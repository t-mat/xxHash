...
#if 1
        /* note : Should we introduce GNU md5sum style espace mark rule for BSD style line?
         * Just say no so far.  We need discussion anyway.
         */
...
static ParseLineResult XSUM_parseLine(ParsedLine* parsedLine, char* line, int rev) {
    /* note : Perhaps, we should refuse escape mark ('\' at the beginning of
     * the line) for BSD style line.  But so far, we always accept it since
     * it's harmless. */
